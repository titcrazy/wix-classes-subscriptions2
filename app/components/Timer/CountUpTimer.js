...
const CountUpTimer = () => {
  ...
  const { years, remainingMonths, remainingDays, remainingHours, remainingMinutes, remainingSeconds } = formatTime(timeElapsed);
  ...
  return (
   <div className="relative mt-5 md:mt:5 rounded-xl">
      <div className="grid grid-cols-3 gap-2 py-4 px-20 md:flex md:items-center md:mt-2 rounded-xl md:px-2 md:py-6 justify-center">
         ...
         <span className="hidden text-3xl -mt-5 md:inline-block md:text-3xl font-normal text-gray-50v">:</span>
         <NumberBox num={remainingMonths} unit="Months" flip={false} />
         <span className="hidden text-3xl -mt-5 md:inline-block md:text-3xl font-normal text-gray-50">:</span>
